...
from lib.analysis import hijack_output, find_module, prepare_calibration_input
...
def get_output_features(model, module):
    with torch.no_grad():
    ...
    for i in range(len(layers)):
        ...
        outputs = []
        handle = hijack_output(target_layer[module], outputs)
        for j in range(nsamples):
            with torch.no_grad():
                outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask, position_ids=position_ids)[0]
        handle.remove()
        torch.cuda.empty_cache()
        features_list.append(torch.cat(outputs, dim=0).mean(dim=0))
...
def draw_heatmap(data, module, mode, original_shape=(2048, 4096), row_num=8, col_num=4):
    ...
    plt.tight_layout()
    plt.savefig(f"figures/output_features/{data_type}_{module}_{mode}_llama_7b.png")
    plt.show()
...
#%%
# Set up logging
logging.basicConfig(filename=f'figures/output_features/{data_type}_llama_7b_seed{seed}.txt', level=logging.INFO)
...
for module in ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "down_proj", "up_proj"]:
    features_list = get_output_features(model, module)
