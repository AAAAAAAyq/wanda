...
from .layerwrapper_old import WrappedGPT, WrappedPlusGPT, SkillGPT, Skill2GPT
...
def prune_bias_unify(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    print("loading calibdation data")
    ...
    for i in range(len(layers)):
        ...
        wrapped_layers = {}
        for name in subset:
            wrapped_layers[name] = Skill2GPT(subset[name])
            # wrapped_layers[name] = SkillGPT(subset[name]) # old version
        ...
        for name in subset:
            ...
            # W_metric = wrapped_layers[name].l2_inp
            # W_metric = torch.sqrt(wrapped_layers[name].l2_inp)
            # W_metric = wrapped_layers[name].l2_inp * torch.norm(subset[name].weight.data, dim=0)
            
            if name == 'self_attn.o_proj':   # 需要组剪枝
                W_metric = wrapped_layers[name].l2_inp
                W_metric = W_metric.reshape(-1, 128).sum(dim=1)
                attn_metric_list.append(W_metric)
                attn_mean_inp_list.append(wrapped_layers[name].mean_inp.type(torch.half))
            else:
                W_metric = torch.sqrt(wrapped_layers[name].l2_inp) * torch.norm(subset[name].weight.data, dim=0)
                mlp_metric_list.append(W_metric)
                mlp_mean_inp_list.append(wrapped_layers[name].mean_inp.type(torch.half))
            # W_metric = torch.abs(subset[name].weight.data) * torch.sqrt(wrapped_layers[name].scaler_inp.reshape((1,-1))) # old version
    ...
    for idx in range(len(layers)):
        if args.remove_heads != 0:
            compress(model, idx, attn_mask[idx], mlp_mask[idx], attn_mean_inp_list[idx], mlp_mean_inp_list[idx], device)
        else:
            compress(model, idx, None, mlp_mask[idx], None, mlp_mean_inp_list[idx], device)
    
    model.config.use_cache = use_cache 
    torch.cuda.empty_cache()

def compress(model, i, attn_mask, mlp_mask, attn_mean_inp, mlp_mean_inp, device):
    # attn
    if attn_mask is not None:
        retain_heads = torch.count_nonzero(attn_mask)
        attn_mask = attn_mask.repeat_interleave(128)
        model.model.layers[i].self_attn.q_proj.weight.data = model.model.layers[i].self_attn.q_proj.weight.data[torch.where(attn_mask)[0]]
        model.model.layers[i].self_attn.q_proj.out_features = attn_mask.sum().item()
        model.model.layers[i].self_attn.k_proj.weight.data = model.model.layers[i].self_attn.k_proj.weight.data[torch.where(attn_mask)[0]]
        model.model.layers[i].self_attn.k_proj.out_features = attn_mask.sum().item()
        model.model.layers[i].self_attn.v_proj.weight.data = model.model.layers[i].self_attn.v_proj.weight.data[torch.where(attn_mask)[0]]
        model.model.layers[i].self_attn.v_proj.out_features = attn_mask.sum().item()
        output_weight = model.model.layers[i].self_attn.o_proj.weight.data
        output_bias = ((attn_mean_inp * ~attn_mask) @ output_weight.T)
        # output_bias = ((wrapped_layers[name].skill_inp.type(torch.half) * ~W_mask) @ output_weight.T) # old version
        output_weight = output_weight[:, torch.where(attn_mask)[0]]
        shape = output_weight.shape
        model.model.layers[i].self_attn.num_heads = retain_heads
        model.model.layers[i].self_attn.hidden_size = retain_heads * 128
    ...
    # mlp
    model.model.layers[i].mlp.up_proj.weight.data = model.model.layers[i].mlp.up_proj.weight.data[torch.where(mlp_mask)[0]]
    model.model.layers[i].mlp.up_proj.out_features = mlp_mask.sum().item()
    model.model.layers[i].mlp.gate_proj.weight.data = model.model.layers[i].mlp.gate_proj.weight.data[torch.where(mlp_mask)[0]]
    model.model.layers[i].mlp.gate_proj.out_features = mlp_mask.sum().item()
...
def prune_with_skill(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            print(f"pruning layer {i} name {name}")
            # W_metric = wrapped_layers[name].l2_inp * torch.sum(subset[name].weight.data.pow(2), dim=0)
            # W_metric = torch.sqrt(wrapped_layers[name].l2_inp) * torch.norm(subset[name].weight.data, dim=0)
            W_metric = wrapped_layers[name].l2_inp
            # W_metric = torch.sqrt(wrapped_layers[name].l2_inp)
            # W_metric = wrapped_layers[name].l2_inp * torch.norm(subset[name].weight.data, dim=0)
            ...
            if name == 'self_attn.o_proj':   # 需要组剪枝
                ...
                output_weight = model.model.layers[i].self_attn.o_proj.weight.data
                output_bias = ((wrapped_layers[name].mean_inp.type(torch.half) * ~W_mask) @ output_weight.T)
                # output_bias = ((wrapped_layers[name].skill_inp.type(torch.half) * ~W_mask) @ output_weight.T) # old version
...
def prune_wanda_sp(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    print("loading calibdation data")
    dataloader, _ = get_loaders("wikitext2",nsamples=args.nsamples,seed=args.seed,seqlen=model.seqlen,tokenizer=tokenizer)
...
@torch.no_grad()
def prune_bias_sparsegpt(args, model, tokenizer, dev, prune_n=0, prune_m=0):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    print("loading calibdation data")
    dataloader, _ = get_loaders("wikitext2",nsamples=args.nsamples,seed=args.seed,seqlen=model.seqlen,tokenizer=tokenizer)
